...
let mesaj =  db.fetch(`otomesaj_${message.guild.id}`)  
  ...
  if(!kanal) return message.channel.send('<a:no:720348931808297033> Otorol Zaten Kapalı')

  db.delete(`otok_${message.guild.id}`)  
  db.delete(`otorol_${message.guild.id}`)   
  db.delete(`otomesaj_${message.guild.id}`)
 message.channel.send('<a:onay:720348580258512896> Otorol **'+message.guild.name+'** Sunucusunda Devre Dışı Bırakılmıştır.')
